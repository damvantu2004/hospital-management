...
export class AuthService {
  ...
  logout(): Observable<any> {
    return this.http.post<any>(this.url + `${API_CONFIG.ENDPOINTS.AUTH.LOGOUT}`, {}).pipe(
      ...
      catchError(error => {
        // Ngay cả khi API lỗi, vẫn clear local data
        this.clearUserData();
  ...
  // CẢI THIỆN: Thêm method check user active
  isUserActive(): boolean {
    const user = this.getCurrentUser();
    return user && user.is_active !== false;
  }

  // CẢI THIỆN: Method clear user data
  ...
  // CẢI THIỆN: Method để check token validity
  isTokenValid(): boolean {
    const token = this.getToken();
    if (!token) return false;

    try {
      // Simple JWT decode để check expiry
