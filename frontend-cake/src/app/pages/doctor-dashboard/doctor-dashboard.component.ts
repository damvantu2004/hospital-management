...
export class DoctorDashboardComponent implements OnInit {
  ...
  processAppointments(appointments: any[]): void {
    ...
    appointments.forEach(apt => {
      console.log(`Appointment ${apt.id}: date = ${apt.appointment_date}, equals today? ${apt.appointment_date === today}`);
    });
    ...
    // Today's appointments với logic mạnh mẽ hơn
    this.todayAppointments = appointments.filter(apt => {
      // Normalize cả 2 ngày về cùng format
      const aptDate = new Date(apt.appointment_date).toISOString().split('T')[0];
      const isToday = aptDate === today;
      
      console.log(`Checking appointment ${apt.id}: ${aptDate} === ${today} ? ${isToday}`);
      return isToday;
    }).sort((a, b) => a.appointment_time.localeCompare(b.appointment_time));

    console.log('Today appointments result:', this.todayAppointments);
  ...
  get isAppointmentsPage(): boolean {
    return this.router.url.includes('/doctor-appointments');
  }
}
